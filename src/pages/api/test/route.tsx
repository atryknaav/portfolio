...
export default async function POST(request: NextApiRequest, res:NextApiResponse<resInt>) {
    ...
    // const { message, email } = await JSON.parse(request);
    const { email, message } = request.body

    // console.log('Got the following: ' + message + email)
    ...
    try {
        ...
        // try{
        // console.log(`Got ${email} and ${message}`)
        // res.status(200).json({ message: `Got ${email} and ${message}` });
        // }
        // catch (err){
        //     console.log(err)
        // }
